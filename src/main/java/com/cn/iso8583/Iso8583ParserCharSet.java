 ...
 * @version 1.0
 */
public class Iso8583ParserCharSet {
	
	private static Logger logger = LoggerFactory.getLogger(Iso8583ParserCharSet.class);
	
	/**
	 * 解析报文
	 * @see Iso8583ParserCharSet#parse(String, byte[], boolean)
	 ...
	 */
	public static Map<String, Object> parse(String suffix, byte[] message, String charset)
			throws Iso8583Exception, IOException, CodecException{
		return parse(suffix,message,true, charset);
	}
	...
	public static Map<String, Object> parse(String suffix, byte[] message, 
			boolean isParseHead, String charset) throws Iso8583Exception, IOException, CodecException{
		...
		parseBody(configuration, is, body, charset);
		return body;
	}
	...
	private static void parseHeader(Iso8583 configuration,
		...
		if (items != null && items.size() > 0) {
			for (Item8583 item : items) {
				String value = parse(is, item, null);
				headMap.put(item.getPropertyName(),value );
				logger.info("添加头域 field_id=[{}],{}=[{}][{}]",item.getFieldId(),
	...
	private static void parseBody(Iso8583 configuration,
			InputStream is,Map<String, Object> fields, String charset) throws IOException, CodecException,
			Iso8583Exception {
		List<Item8583> items = configuration.getBody();
		Item8583 item = items.get(0);
		logger.info(item.toString());
		String value = parse(is, item, charset);
	...
	private static String parse(InputStream is, Item8583 field, String charset)
		...
		if (data_type.equals(Iso8583Constants.DATA_TYPE_CHARASCII)
			...
			if (field.getConvertor() != null
				...
				if(null != charset)
					data_value = new String(retBytes, charset);
				else
