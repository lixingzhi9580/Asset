...
public class SAXUnionFiled55Utils {
	 ...
	 * @return tlv map
	 */
	public static Map<String, String> getUnionField55Map(String hexfiled55) {
		if (null == hexfiled55) {
			throw new IllegalArgumentException("55域的值不能为空!");
		}
		return builderKeyAndVal(hexfiled55);
	}

	public static Map<String, String> builderKeyAndVal(String hexString) {
	...
	/**
	 * 解析117域的信息，返回域的各tag值是String而不是hex值
	 * @param hexfiled
	 * @return
	 * @throws CodecException 
	 */
	public static Map<String, String> getUnionField117Map(String hexfiled117) throws CodecException {
		if (null == hexfiled117) {
			throw new IllegalArgumentException("域的值不能为空!");
		}
		return builderKeyAndStringVal(hexfiled117);
	}

	public static Map<String, String> builderKeyAndStringVal(String hexString) throws CodecException {
		Map<String, String> tlvs = new HashMap<String, String>();
		int position = 0;
		while (position != hexString.length()) {
			String _hexTag = getUnionTag(hexString, position);
			...
			String _value = hexString.substring(position, position + _vl * 2);
			position = position + _value.length();
			tlvs.put(_hexTag, new String(AsciiCodec.hexToByteArray(_value)));
		}
		return tlvs;
	}
