package com.cn.http.httpclient.builder;
...
import com.cn.http.common.SSLs;
...
public class  HCB extends HttpClientBuilder{
	...
	public static void main(String[] args) throws HttpProcessException {
		String url="https://svrapi.test.webank.com/l/api/oauth2/access_token?grant_type=client_credential&secret=sedfasdgfdhweqrferrg&app_id=wz123456&version=1.0.0";
		HttpConfig  config = HttpConfig.custom().url(url);
		HttpClient client= HCB.custom().timeout(10000).ssl().build();//采用默认方式（绕过证书验证）
		//执行请求
		String resp = HttpClientUtil.get(config.client(client));
