...
public class Parse8583 {
	 ...
	 * @return
	 */
	public static String formatBitMap(String bits) {
		StringBuilder sb0 = new StringBuilder("\r\n|");
		StringBuilder sb1 = new StringBuilder("\r\n|");
		char[] chars = bits.toCharArray();
		for (int i = 1; i <= chars.length; i++) {
			sb0.append(StringUtils.leftPad(i + "", 3)).append("|");
			sb1.append(StringUtils.leftPad(chars[i - 1] + "", 3)).append("|");
		}
	 ...
	 */
	static {
		String path = "def8583" + File.separator+"POS_8583.xml";
		try {
			InputStream is= new ClassPathResource(path).getInputStream();
			SAXReader saxReader = new SAXReader();
			Element def8583= saxReader.read(is).getRootElement();

			encoding = def8583.attributeValue("encoding");
			encoding = null == encoding ? "UTF-8" : encoding;
			LOG.debug("8583报文字符集[{}]" ,encoding);
	...
	public static byte[] encode(Map<String, Object> map) {
		...
		try {
			...
			LOG.error(e.getMessage(), e);
			if (null != field) {
				LOG.error("解析第[{}]域异常", field.getId(),e);
			}else{
				LOG.error(e.getMessage(), e);
			}
		} finally {
	...
	public static byte[] encodeValue(Field8583 field, Object value) throws IOException {
		...
		if ("const".equals(field.getType())) {
			if ("byte".equals(field.getSourceType())) {
				...
				if (bytes.length != field.getSourceLength())
					LOG.error("解析第[" + field.getId() + "]域，实际长度" + bytes.length + ",sourceLength=" + field.getSourceLength());
			} else {
				String strValue = ((String) value);
				if (0 < field.getLength() && strValue.length() != field.getLength()) {
					LOG.error("解析第[" + field.getId() + "]域，实际长度" + strValue.length() + ",length=" + field.getLength());
				}
			...
			if ("var3".equals(field.getType()) && 10 > Integer.parseInt(len))
				len = "0" + len;
			if ("var3".equals(field.getType()) && 100 > Integer.parseInt(len))
				len = "0" + len;
			if ("var3".equals(field.getType()) && 1000 > Integer.parseInt(len))
				len = "0" + len;
		...
		if ("2".equals(field.getId()) || "14".equals(field.getId())
				|| "35".equals(field.getId()) || "36".equals(field.getId())
				|| "52".equals(field.getId())) {
			LOG.debug("enocde域[{}],{}=[{}][******][***********]", field.getId(),
					field.getName(), bytes.length, value,
	...
	public static Map<String, Object> decode(byte[] bytes) {
		...
		try {
			...
			LOG.debug("消息类型：[{}]", map.get(field.getName()));
			...
			LOG.debug("位图：[{}]", bitMapHex);
			LOG.debug("位图：[{}]", bitMapStr);
			field = FIELDS.get(1);
			...
			LOG.error(e.getMessage(), e);
			if (null != field) {
				LOG.error("解析第[" + field.getId() + "]域异常", e);
			}else{
				LOG.error("8583报文解析出错", e);
			}
		} finally {
			try {
				stream.close();
	...
	private static Object decodeValue(Field8583 field, InputStream stream) throws IOException {
		...
		if (field.getType().startsWith("var")) {
			if ("var3".equals(field.getType())) {
				...
				LOG.error("解析第[" + field.getId() + "]域，不存在的type=" + field.getType());
		...
		LOG.debug("decode域[{}],{}=[{}][{}][{}]", field.getId(), field.getName(), rlen, value, ByteUtil.bytesToHex(bytes));
